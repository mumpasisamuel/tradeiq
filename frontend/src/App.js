    ...
    if (isCrypto) {
      fetch(`https://api.binance.com/api/v3/klines?symbol=${asset.id}&interval=1m&limit=60`)
        .then(r => r.json()).then(data => {
          ...
          setHistory(pts); setPrice(pts[pts.length-1]?.v);
      ...
      wsRef.current.onmessage = (e) => {
        ...
        setHistory(h => {
          ...
          const f = upd[0]?.v; if (f) setChange(((p-f)/f)*100);
          return upd;
        });
      };
    } else {
      ...
      for (let i=59;i>=0;i--) { p=p*(1+(Math.random()-0.495)*0.003); hist.push({t:Date.now()-i*60000,v:parseFloat(p.toFixed(5))}); }
      ...
      const iv = setInterval(() => {
        setPrice(prev => {
          ...
          setHistory(h => { const upd=[...h.slice(-59),{t:Date.now(),v:next}]; setChange(((upd[upd.length-1].v-upd[0].v)/upd[0].v)*100); return upd; });
          return next;
        });
      }, 3000);
...
function analyze(history, style) {
  ...
  if(score>=thr){ signal="BUY"; color="#00e676"; sl=parseFloat((cur*0.98).toFixed(6)); tp=parseFloat((cur*1.04).toFixed(6)); advice=score>=6?"Signal FORT d'achat ✅":"Signal modéré d'achat"; }
  else if(score<=-thr){ signal="SELL"; color="#ff1744"; sl=parseFloat((cur*1.02).toFixed(6)); tp=parseFloat((cur*0.96).toFixed(6)); advice=score<=-6?"Signal FORT de vente 🔴":"Signal modéré de vente"; }
...
export default function App() {
  ...
  useEffect(() => { if (mode === "reel") fetchBalance(); }, [mode]);
  ...
  const placeOrder = async (side) => {
    ...
    try {
      ...
      const data = await res.json();
      setOrders(o => [{id:data.orderId||`ORD-${Date.now()}`,side,symbol:asset.symbol,qty,price,time:new Date().toLocaleTimeString("fr-FR"),status:data.status||"FILLED",mode}, ...o.slice(0,19)]);
      setOrderMsg("success");
      if (mode==="reel") fetchBalance();
    } catch {
      setOrders(o => [{id:`SIM-${Date.now()}`,side,symbol:asset.symbol,qty,price,time:new Date().toLocaleTimeString("fr-FR"),status:"SIMULÉ",mode:"demo"}, ...o.slice(0,19)]);
  ...
  const toggleBot = async () => {
    ...
    } catch { setBotRunning(!botRunning); }
  };
      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"0 12px"}}>
        ...
        {mode==="reel"&&(
          <div style={{background:"#0a1520",border:"1px solid #1e2a3a",borderRadius:12,padding:"12px 14px",marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
              <div style={{fontSize:11,fontWeight:700,color:"#cdd9e5"}}>💰 Solde Binance</div>
              <button onClick={fetchBalance} style={{background:"none",border:"1px solid #1e2a3a",borderRadius:6,color:"#00b4d8",fontSize:10,padding:"3px 8px"}}>{balanceLoading?"⟳ ...":"↻ Actualiser"}</button>
            </div>
            {balance?(
              <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                {Object.entries(balance).slice(0,6).map(([k,v])=>(
                  <div key={k} style={{background:"#0d1b2a",borderRadius:8,padding:"6px 10px",minWidth:80}}>
                    ...
                    <div style={{fontFamily:"'Space Mono',monospace",fontSize:12,color:"#00e676",fontWeight:700}}>{parseFloat(v).toFixed(4)}</div>
                  </div>
                ))}
              </div>
            ):<div style={{fontSize:11,color:"#4a6075"}}>{balanceLoading?"Chargement...":"Clique Actualiser pour voir ton solde"}</div>}
          </div>
        )}
        ...
        <div style={{background:"#0a1520",border:`1px solid ${botRunning?"#00e676":"#1e2a3a"}`,borderRadius:12,padding:"12px 14px",marginBottom:10,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            ...
            <div style={{fontSize:10,color:botRunning?"#00e676":"#4a6075"}}>{botRunning?"Actif — trading en cours":"Inactif"}</div>
          </div>
          <button onClick={toggleBot} style={{padding:"8px 16px",borderRadius:8,border:"none",fontWeight:700,fontSize:12,background:botRunning?"#ff1744":"#00e676",color:"#000"}}>{botRunning?"⏹ STOP":"▶ START"}</button>
        </div>
        ...
        <div className="fade" key={asset?.id} style={{background:"#0a1520",border:"1px solid #1e2a3a",borderRadius:16,padding:"16px",marginBottom:12}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
            ...
            <div style={{padding:"5px 10px",borderRadius:20,background:change>=0?"#00e67615":"#ff174415",color:change>=0?"#00e676":"#ff1744",fontFamily:"'Space Mono',monospace",fontSize:12,fontWeight:700}}>{change>=0?"+":""}{change.toFixed(2)}%</div>
          </div>
          <Sparkline history={history} color={change>=0?"#00e676":"#ff1744"}/>
          <div style={{fontSize:9,color:"#2a3a4a",marginTop:4}}>{status==="live"?"WebSocket Binance · temps réel":"Mode démo"}</div>
        </div>

        <div style={{display:"flex",gap:8,marginBottom:12}}>
          {STYLES.map(s=>(
            <button key={s.id} onClick={()=>setStyle(s.id)} style={{flex:1,padding:"9px 4px",borderRadius:10,border:"1px solid",textAlign:"center",color:style===s.id?"#060c13":"#8899aa",background:style===s.id?"#ffd600":"transparent",borderColor:style===s.id?"#ffd600":"#1e2a3a",transition:"all .2s"}}>
        ...
        {analysis&&(
          <div className="fade" style={{background:`linear-gradient(135deg,${analysis.color}12,${analysis.color}04)`,border:`2px solid ${analysis.color}`,borderRadius:16,padding:"16px",marginBottom:12,display:"flex",alignItems:"center",gap:14}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:analysis.color,letterSpacing:2,marginBottom:3}}>{analysis.signal==="BUY"?"SIGNAL D'ACHAT":analysis.signal==="SELL"?"SIGNAL DE VENTE":"ATTENDRE"}</div>
        ...
        <div style={{background:"#0a1520",border:"1px solid #1e2a3a",borderRadius:16,padding:"14px",marginBottom:12}}>
          <div style={{fontSize:11,fontWeight:700,color:"#cdd9e5",marginBottom:6}}>🎯 Passer un ordre {mode==="demo"?"(simulation)":"(RÉEL ⚠️)"}</div>
          {mode==="reel"&&<div style={{background:"#ff174415",border:"1px solid #ff174430",borderRadius:8,padding:"8px",marginBottom:8,fontSize:10,color:"#ff9999"}}>⚠️ Mode RÉEL — les ordres utilisent ton vrai argent Binance !</div>}
          ...
          {orderMsg==="simulated"&&<div style={{textAlign:"center",color:"#ffd600",fontSize:11,padding:"6px",marginBottom:8,background:"#ffd60010",borderRadius:8}}>🟡 Ordre simulé</div>}
        ...
        {tab==="signal"&&analysis&&(
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            <div style={{background:"#0a1520",border:"1px solid #1e2a3a",borderRadius:12,padding:"12px"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:"#4a6075",marginTop:3}}><span>Survente ✅</span><span>Surachat ❌</span></div>
            </div>
            <div style={{background:"#0a1520",border:"1px solid #1e2a3a",borderRadius:12,padding:"12px"}}>
              <div style={{fontSize:11,fontWeight:700,color:"#cdd9e5",marginBottom:8}}>Moyennes Mobiles EMA</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
                {[{l:"EMA 9",v:fmt(analysis.ema9),c:"#00b4d8"},{l:"EMA 20",v:fmt(analysis.ema20),c:"#ffd600"},{l:"Signal",v:analysis.ema9>analysis.ema20?"↑ HAUSSE":"↓ BAISSE",c:analysis.ema9>analysis.ema20?"#00e676":"#ff1744"}].map(i=>(
        ...
        {tab==="orders"&&(
          <div>
            {orders.length===0?(
              <div style={{textAlign:"center",color:"#4a6075",padding:"30px 0",fontSize:12}}>Aucun ordre encore.<br/><span style={{fontSize:10}}>Clique BUY ou SELL.</span></div>
            ):orders.map(o=>(
